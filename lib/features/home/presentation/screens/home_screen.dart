...
class _HomeScreenState extends State<HomeScreen> {
  ...
  List<Map<String, dynamic>> _getCategories(bool isArabic) {
    return [
      {
        'title': isArabic ? 'كل الملاعب' : 'All Fields',
        'rawKey': 'كل الملاعب',
        'icon': Icons.sports_soccer_rounded,
      },
      {
        'title': isArabic ? 'خماسي' : '5v5',
        'rawKey': 'خماسي',
        'icon': Icons.groups_rounded,
      },
      {
        'title': isArabic ? 'سباعي' : '7v7',
        'rawKey': 'سباعي',
        'icon': Icons.stadium_rounded,
      },
      {'title': '11v11', 'rawKey': '11v11', 'icon': Icons.flag_rounded},
      {
        'title': isArabic ? 'صالات' : 'Indoor',
        'rawKey': 'صالات',
  ...
  void _toggleFavorite(BuildContext context, String fieldId) async {
    await _homeCubit.toggleFavorite(fieldId);
    if (context.mounted) {
      try {
        _favoritesCubit.loadFavorites(isRefresh: true);
      } catch (_) {}
    }
  }
        ...
        child: BlocListener<FavoritesCubit, FavoritesState>(
          listener: (context, favState) {
            if (favState is FavoritesLoaded) {
              _homeCubit.syncFavorites(favState.favoriteIds);
            }
          },
          child: BlocBuilder<HomeCubit, HomeState>(
            builder: (context, state) {
              if (state is HomeLoading) {
                return _buildLoadingState(isDark: isDark, size: size);
              }
              ...
              if (state is HomeLoaded) {
                return RefreshIndicator(
                  onRefresh: () async {
                    await _homeCubit.loadHomeData(isRefresh: true);
                    if (context.mounted) {
                      try {
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        HomeSearchBar(
                          ...
                          onChanged: (query) {
                            _homeCubit.searchFields(query);
                          },
                          filterParams: state.filterParams,
                          onFilterApplied: (params) {
                            _homeCubit.applyFilter(params);
                          },
                        ),
                        SizedBox(height: 20.h),
                        ...
                        SizedBox(
                          ...
                          child: ListView.separated(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final isSelected =
                                  state.selectedCategory == rawKey ||
                                  state.selectedCategory == displayTitle;
                              ...
                              return CategoryChip(
                                ...
                                onTap: () {
                                  _homeCubit.selectCategory(rawKey);
                                },
                              );
                            },
                          ),
                            ...
                            children: [
                              ...
                              TextButton(
                                onPressed: () {
                                  ...
                                  _homeCubit.selectCategory('كل الملاعب');
                                },
                                child: Text(
                          ...
                          if (state.filteredFields.isEmpty)
                            ...
                            ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final isFav = state.favoriteFieldIds.contains(
                                  field.id,
                                );
                                return FieldCard(
                                  field: field,
  ...
  Widget _buildLoadingState({required bool isDark, required Size size}) {
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 20.h),
          LoadingSkeleton(width: size.width, height: 50.h, borderRadius: 16),
          SizedBox(height: 20.h),
          LoadingSkeleton(width: size.width, height: 155.h, borderRadius: 22),
          SizedBox(height: 24.h),
          Column(
    ...
    return Container(
      ...
      child: Column(
        children: [
          ...
          Text(
            isArabic
                ? 'لا توجد ملاعب مطابقة لمحددات البحث'
          ...
          Text(
            isArabic
                ? 'جرب تغيير كلمة البحث أو فلاتر التصفية.'
